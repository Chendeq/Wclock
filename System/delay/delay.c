...
/**
 * @brief     初始化延迟函数
 * @param     sysclk: 系统时钟频率, 即CPU频率(rcc_c_ck), 单位为MHz, 例如168MHz传168
 * @retval    无
 */  
void delay_init(uint16_t sysclk)
{
    ...
    uint32_t reload;
#endif
    SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK);/* SYSTICK使用内核时钟源,同CPU同频率 */
 ...
 * @brief     延时nus
 * @param     nus: 要延时的us数
 * @note      nus取值范围: 0~8947848(最大值即2^32 / fac_us @fac_us = 168)
 * @retval    无
 */ 
