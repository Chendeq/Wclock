 ...
 * @date       :2018-08-09
 * @function   :通过SPI总线向触摸屏IC写入一个字节数据
 * @parameters :num:要写入的数据
 * @retvalue   :无
 ******************************************************************************/
 ...
 * @date       :2018-08-09
 * @function   :通过SPI总线从触摸屏IC读取ADC值
 * @parameters :CMD:读取命令,0xD0为x坐标,0x90为y坐标
 * @retvalue   :读取的数据
 ******************************************************************************/
 ...
 * @date       :2018-08-09
 * @function   :读取触摸屏坐标(x或y),
								连续读取READ_TIMES次数据并对数据进行升序排序,
								然后去掉最低和最高的LOST_VAL个,取平均值
 * @parameters :xy:读取命令(CMD_RDX/CMD_RDY)
 * @retvalue   :读取的数据
******************************************************************************/
 ...
 * @date       :2018-08-09
 * @function   :连续读取两次触摸屏坐标,
								且这两次的偏差不能超过ERR_RANGE,
								满足条件,则认为读取正确,
								否则认为读取错误.
								该函数可以大大提高准确度.
 * @parameters :x:读取的触摸屏x坐标
...
//////////////////////////////////////////////////////////////////////////
// 保存在EEPROM里面的地址空间,占用13个字节(RANGE:SAVE_ADDR_BASE~SAVE_ADDR_BASE+12)
...
void TP_Save_Adjdata(void)
{
	s32 temp;
	// 保存校正结果!
	temp = tp_dev.xfac * 100000000; // 保存x校正因子
	AT24CXX_WriteLenByte(SAVE_ADDR_BASE, temp, 4);
	temp = tp_dev.yfac * 100000000; // 保存y校正因子
...
uint8_t TP_Get_Adjdata(void)
{
	...
	if (tempfac == 0X0A)								// 触摸屏已经校准过了
	{
		...
		tp_dev.touchtype = AT24CXX_ReadOneByte(SAVE_ADDR_BASE + 12); // 读取触摸类型标记
...
/*****************************************************************************
 * @name       :uint8_t TP_Adjdata(void)
 * @date       :2018-08-09
 * @function   :校准触摸屏并获取4个校准参数
 * @parameters :无
 * @retvalue   :无
 ******************************************************************************/
void TP_Adjust(void)
{
	uint16_t pos_temp[4][2]; // 坐标存储值
	uint8_t cnt = 0;
	uint16_t d1, d2;
	uint32_t tem1, tem2;
	double fac;
	...
	LCD_Clear(WHITE);  // 清屏
	POINT_COLOR = BLACK;
	LCD_ShowString(10, 40, 16, (uint8_t *)TP_REMIND_MSG_TBL, 1);			 // 显示提示信息
	LCD_ShowString(10, 56, 16, (uint8_t *)"the cross on the screen.", 1);	 // 显示提示信息
	LCD_ShowString(10, 72, 16, (uint8_t *)"The cross will always move", 1);	 // 显示提示信息
	LCD_ShowString(10, 88, 16, (uint8_t *)"until the screen adjustment", 1); // 显示提示信息
	LCD_ShowString(10, 104, 16, (uint8_t *)"is completed.", 1);				 // 显示提示信息

	TP_Drow_Touch_Point(20, 20, RED); // 画点1
	tp_dev.sta = 0;					  // 清除触发信号
	tp_dev.xfac = 0;				  // xfac用来标记是否校准过,所以校准之前必须清零!以免错误
	while (1)						  // 如果连续10秒钟没有按下,则自动退出
	{
		...
		if ((tp_dev.sta & 0xc0) == TP_CATH_PRES) // 按键按下了一次(此时按键松开了.)
		{
			...
			case 4: // 全部四个点已经得到
				...
				if (abs(tp_dev.xfac) > 2 || abs(tp_dev.yfac) > 2)									 // 触摸和预设的相反了.
				{
					cnt = 0;
					TP_Drow_Touch_Point(lcddev.width - 20, lcddev.height - 20, WHITE); // 清除点4
					TP_Drow_Touch_Point(20, 20, RED);								   // 画点1
...
uint8_t TP_Init(void)
{
	...
	//	TP_Get_Adjdata();
	//	return 1;
	if (TP_Get_Adjdata()) // 成功读取校准参数
		return 1;		  // 已经校准（从EEPROM读取）
	else				  // 未读取到校准参数，需要校准
	{
		LCD_Clear(WHITE); // 清屏
		TP_Adjust();	  // 屏幕校准
